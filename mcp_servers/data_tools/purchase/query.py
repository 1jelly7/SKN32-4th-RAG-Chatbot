...
from mcp_servers.data_tools.purchase.schema import SchemaResource, get_schema_resource
...
#     ]
#
# 변경 이유: mcp_servers/data_tools/sales/query.py와 동일 — 복합질문에서는 항목
# (하위 SELECT)마다 빈 결과 evidence를 하나씩 만들어야 해서, dict 조립 자체는
# _empty_evidence_item()으로 옮기고 label을 추가했다. 이 함수는 항목이 없는
# 최상단 실패(빈 질문·NO_SQL 전체)에서 기존과 동일하게 그 dict를 리스트 1개로
# 감싸는 역할만 한다.
...
# 처리하는 헬퍼. mcp_servers/data_tools/sales/query.py와 동일 — old query_purchase()에
# 있던 단일 쿼리 파이프라인을 그대로 옮긴 것으로, validate_and_normalize ->
# explain_readonly -> 실패 시 generate_sql_with_error 1회 재작성 -> query_readonly
# 순서와 판단 로직은 한 글자도 바꾸지 않았다. 항목별로 이 함수를 asyncio.gather로
# 동시에 돌리기 위해 별도 함수로 뺐다.
async def _run_single_query(
    ...
    try:
        ...
        if not retried:
            ...
            return _empty_evidence_item(
                ...
                elapsed_ms,
                retry_count,
                "요청 조건을 구매 데이터 조회로 해석할 수 없습니다. 기간·공급업체·구매액 기준을 구체적으로 알려 주세요.",
            )
    ...
    if not rows:
        return _empty_evidence_item(
            ...
            elapsed_ms,
            retry_count,
            "해당 조건의 구매 데이터가 없습니다. 보유 기간과 조건을 확인해 다시 질문해 주세요.",
        )
