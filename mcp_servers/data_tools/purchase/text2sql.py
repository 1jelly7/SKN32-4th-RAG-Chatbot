...
# 추가: 복합질문(예: "올해 구매액과 구매액 최대 공급업체")을 여러 개의 독립된 단일
# SELECT로 쪼갤 때 허용하는 최대 개수. mcp_servers/data_tools/sales/text2sql.py와
# 동일한 이유·값(무제한 허용 시 비용·응답시간 폭주 방지).
MAX_SUB_QUERIES = 3
...
SYSTEM_PROMPT = (
    ...
    # 변경 이유: mcp_servers/data_tools/sales/text2sql.py와 동일 — 복합질문을 하나의
...
def _extract_queries(raw: str) -> list[dict[str, str]]:
    ...
    try:
        ...
        logger.warning("purchase text2sql 응답 JSON 파싱 실패: %s / raw=%r", exc, raw)
        return []
    if not isinstance(queries, list):
        logger.warning("purchase text2sql queries가 리스트가 아님: %r", queries)
        return []
    return [
