...
python -m pytest tests/unit/test_sales_text2sql.py tests/unit/test_purchase_text2sql.py tests/unit/test_agent.py
```
...
| 테스트 | 검증 내용 |
|---|---|
| `tests/unit/test_api.py` | liveness health, 정적 UI no-store, 문서 ID 다운로드, cache hit/miss, 요청·timing 헤더, 입력 검증, 공개 오류 매핑 |
| `tests/unit/test_auth.py` | scrypt 해시, login/me/logout, 비활성 계정, 역할별 DB 정책, HR의 데이터 질의 차단, 사용자별 cache 격리 |
| `tests/unit/test_web_auth.py` | 로그인 상태 복원, logout 정리, 로컬 Chart.js, 차트 크기, 다운로드 파일명 처리 |
...
| 테스트 | 검증 내용 |
|---|---|
| `tests/unit/test_agent.py` | route/data domain, 순차 BOTH 병합, 부분 실패, evidence 상태·정책·충돌, 1회 재조회, LLM evidence sanitization, 출처·표 직렬화 |
...
앞의 네 chat/cache 통합 파일은 `FakeMCPPort`, fake LLM, 메모리 계정·세션·캐시를 사용한다.
외부 MCP 프로세스, 실제 OpenAI, MySQL, Redis, 운영 FAISS를 호출하지 않는다.

## 현재 자동 테스트가 보장하는 계약

### TS-A01: 인증과 RBAC

- 보호 API는 유효한 서명 쿠키와 활성 서버 세션 없이는 `401`이다.
...
- `BOTH`는 Document 후 Database 순서로 실행하고 evidence 유형을 분리한다.
...
### TS-T07: `scripts/setup_all.py` 초기화
...
- 초기 계정 시딩은 별도 단계임이 명확하다.
- 재실행이 기존 데이터와 설정을 손상하지 않는다.

현재 스크립트는 이동 후 상대경로, 고정 MySQL 설치 경로와 계정 전제가 있어 이 수용
테스트를 통과한 것으로 간주하지 않는다.
...
| D04 | `finance` | 규정+매출 복합 질문 | 순차 BOTH, 도메인별 근거, 부분 실패 |
