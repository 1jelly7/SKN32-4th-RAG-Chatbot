...
## 4-1. (2026-08-03 추가) 실제 화면에서 재현한 문제 2건

PM이 실제 채팅 화면에서 "지난 5년동안 월별 매출 그래프 그려줘"를 테스트하다가 발견했다.
sales가 직접 고칠 수 없는 파일(`nodes.py`, `chat.js`, `style.css`)이라 여기에 요청으로
남긴다.

### (A) 60개월 시계열 질문에서 표만 나오고 차트가 안 그려짐

- **근거**: [app/agent/nodes.py:221](../../app/agent/nodes.py:221)
  ```python
  chartable = label_column is not None and value_column is not None and len(rows) <= 30
  ```
  5년 월별 집계는 60행이라 이 상한(`<= 30`)을 넘어 `chartable=False`가 된다. 반면 sales
  프롬프트는 시계열 질문에 대해 최대 60개까지 허용하도록 이미 설계돼 있다(3-2절, sales
  `text2sql.py` 규칙 11). 즉 sales가 만든 데이터 상한과 backend의 차트 상한이 서로
  맞지 않는다.
- **요청 내용**: `chartable` 조건의 행 수 상한을 sales의 시계열 상한(60)에 맞추거나,
  라벨 컬럼 종류(기간 vs 카테고리)에 따라 상한을 다르게 적용(카테고리 12 / 시계열 60).
- **완료 기준**: "지난 5년 월별 매출 추이" 질문에서 표뿐 아니라 꺾은선 그래프도 함께
  그려짐.
...
- **요청 내용**: `.chart-wrap`에 고정 높이(예: `height: 320px`) 추가 + `drawChart` 옵션에
  `maintainAspectRatio: false` 추가. 둘 중 하나만 해도 되지만 함께 하는 걸 권장한다.
- **완료 기준**: 채팅창 크기를 이리저리 바꿔도 차트 크기가 진동 없이 안정적으로 유지됨.

## 5. 검증 기준

> 2026-08-03 반영: 아래 기능을 구현하고 단위 테스트로 구조적 회귀를 검증했다.
> 실제 브라우저에서의 리사이즈와 오프라인 렌더링은 배포 번들 환경에서 추가 확인한다.
...
- [ ] "지난 5년 월별 매출 추이"(60행)에서도 차트가 그려진다 (4-1-A 해결 확인)
- [ ] 채팅창 크기를 바꿔도 차트 크기가 진동하지 않는다 (4-1-B 해결 확인)
