...
> 2026-08-03 통합 검토·반영: P0-3의 코드 연결, P1-3~P1-6, P1-8, P1-10, P1-11 및
> 04-A·04-B를 반영하고 단위 테스트로 검증했다. 실제 purchase DB 권한 부여와 ETL
> freshness 공급자는 외부 운영 환경/도메인 ETL 계약이 필요해 보류한다.
...
> 질문이 502가 됨)과 4-1절 차트 버그 2건(04_chart_spec.md)을 발견해 추가했다. 팀원
...
### (완료) 구 P1-9. `mcp_servers/data/`와 `mcp_servers/data_tools/` 중복 정리

- **상태**: 이미 해결돼 있었다. `mcp_servers/data/`는 커밋 `367833c`(2026-08-02)에서
  통합 담당이 이미 삭제했다(PROGRESS.md Key Decisions에도 "정본 backend Tool 경계를
  `document_tools`/`data_tools`로 한정하고 중복 패키지를 제거한다"로 기록돼 있음).
  지금 저장소에는 `mcp_servers/data_tools/`와 `mcp_servers/document_tools/`만 있다.
  **주의**: 이 항목은 구현 착수 전 초안에서 그대로 옮겨 적은 것으로, 이번 "실제 구현
  반영" 재작성 때 재검증하지 않고 넘어갔던 실수다(2026-08-03 sales 팀원이 지적해서
  발견).
...
> **오해 방지**: 이 항목은 **화면에 보여주는 표를 줄이자는 게 아니다.** 화면 표는
> 지금도 `rows`(최대 200행)를 그대로 다 보여준다. 여기서 자르자는 대상은 딱 하나,
> `answer_synthesis`가 답변 문장을 만들 때 OpenAI에 보내는 프롬프트다 — 화면 렌더링용
> `tables`와 답변 생성용 evidence는 이미 별개 응답 필드(`data.tables` vs `data.answer`)로
> 나가고 있고, 여기서 안 되어 있는 건 "LLM 프롬프트를 만들 때 `rows`를 자르지 않고
> 통째로 넣는다"는 것뿐이다.
...
> **두 가지가 섞여 있다**: (1) 문서(`docs/interface.md`)가 낡아서 실제 필드명과 다름 —
> 단순 오타 수준. (2) 더 중요한 건, `query_sales`가 계산한 `views_used`/`data_coverage`/
> `chart_hint` 같은 정보를 `server.py`가 응답 단계에서 대부분 버리고 있다는 것 — 문서를
> 아무리 고쳐도 코드가 이 값들을 안 보내면 의미가 없다. 그래서 요청 순서도 코드 확장(1)이
> 먼저고 문서 갱신(2)이 그 다음이다.
...
| 코드 | 최신 상태 | 처리·근거 |
|---|---|---|
| P0-3 | 코드 완료, 운영 권한 확인 대기 | purchase MySQL adapter가 전용 조회 계정을 우선 사용하도록 연결했다. 실제 `purchase.*` SELECT 권한은 저장소에서 검증할 수 없다. |
| P1-3 | 완료 | 표 내부 가로 스크롤과 콘텐츠 폭 최소값을 적용했다. |
| P1-4 | 완료 | `NO_RESULT`의 안내 문구를 그래프 상태에 보존해 최종 답변으로 반환한다. |
| P1-5 | 완료 | 숫자를 콤마·소수점 둘째 자리로, sales 표·툴팁은 JOD로 표시한다. |
| P1-6 | 완료 | 화면 표는 보존하고 LLM 프롬프트의 DB 행만 50행으로 제한한다. |
| P1-7 | 미해결 | ETL 완료 시각/버전 공급 계약과 저장 위치가 아직 없다. 임의 파일·DB 직접 접근은 추가하지 않는다. |
| P1-8 | 완료 | 공통 envelope가 도메인 metadata를 보존하고, interface 문서를 실제 필드명으로 갱신했다. |
| P1-10 | 완료 | 도메인별 빈 결과 message를 envelope와 최종 답변까지 전달한다. sales는 범위 밖 지표와 실제 0건을 구분한다. |
| P1-11, 04-A, 04-B | 완료 | 막대/꺾은선, 정수 기간 라벨, 60개 시계열, 로컬 Chart.js, 축·툴팁 포맷, 안정 크기를 구현했다. |
| P2-1 | 미착수 | MCP 전용 로그의 내용·보존 정책은 도메인 소유자와 합의가 필요하다. 기존 `logs/` 무시 규칙은 이미 적용된다. |
| P2-2 | 이미 해결 | `purchase_db_database` 필드가 이미 사용 중이며 `finance_db_database`는 현재 저장소에 없다. |
...
| P0-3 | purchase DB 접근 거부(502) — `purchase_read_user` 연결 필요 (신규) | 통합/purchase | P0 | 코드 완료, DB 권한 확인 대기 |
| P1-3 | 표 가로 스크롤 CSS | 통합 | P1 | 완료 |
| P1-4 | 0건 안내 문구가 최종 답변에 반영되는지 확인 | 통합 | P1 | 완료 |
| P1-5 | 표·답변 숫자 표기 통일 | 통합 | P1 | 완료 |
| P1-6 | LLM 프롬프트용 행 수를 화면과 분리 | 통합 | P1 | 완료 |
| P1-7 | 캐시 freshness bucket 채우기 | 통합 | P1 | ETL 계약 대기 |
| P1-8 | interface.md/metadata 실제 필드명으로 갱신 | 통합 | P1 | 완료 |
| P1-10 | server.py envelope가 거절 사유·안내 문장 전달하도록 확장 (신규) | 통합 | P1 | 완료 |
| P1-11 | 채팅 그래프(막대+꺾은선) UI 구현, 스펙은 [04_chart_spec.md](04_chart_spec.md) (신규) | 통합 | P1 | 완료 |
| 04-A | 60행 시계열 질문에서 차트 안 그려짐 (`nodes.py` chartable 상한 30) (신규) | 통합 | P1 | 완료 |
| 04-B | 차트 크기가 저절로 진동함 | 통합 | P1 | 완료 |
| P2-1 | MCP 조회 로그 파일 정책 추가 | 통합 | P2 | 미착수 |
| P2-2 | finance_db_database 필드명 정리 | 통합 | P2 | 이미 해결 |
