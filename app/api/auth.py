...
from app.auth.sessions import issue_session, read_session
...
async def login(body: LoginRequest, request: Request, response: Response) -> LoginResponse:
    ...
    session_id = read_session(token, request.app.state.auth_secret)["sid"]
    request.app.state.dependencies.session_store.issue(str(session_id))
...
async def logout(response: Response, user: CurrentUser, request: Request) -> None:
    """브라우저 세션 쿠키를 즉시 제거하여 후속 보호 요청을 차단한다."""
    request.app.state.dependencies.session_store.revoke(str(user["session_id"]))
