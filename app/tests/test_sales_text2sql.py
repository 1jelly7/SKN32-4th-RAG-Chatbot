    ...
    raw = json.dumps(
        {
            "queries": [
                {
                    "label": "올해 매출 합계",
                    "sql": "SELECT SUM(order_amount) AS total FROM v_sales_order",
                },
                {
                    "label": "매출 최고 기업",
                    "sql": "SELECT customer_name FROM v_sales_order ORDER BY order_amount DESC LIMIT 1",
                },
            ]
        }
    )
...
def test_extract_queries_truncates_to_max_sub_queries() -> None:
    ...
    raw = json.dumps(
        {
            "queries": [
                {"label": f"항목{i}", "sql": f"SELECT {i} FROM v_sales_order"}
                for i in range(5)
            ]
        }
    )
    ...
    raw = json.dumps(
        {
            "queries": [
                {"label": "빈 항목"},
                {"label": "정상", "sql": "SELECT 1 FROM v_sales_order"},
            ]
        }
    )
    assert text2sql_module._extract_queries(raw) == [
...
def test_query_sales_retries_once_on_explain_failure() -> None:
    ...
    async def first_sql(question: str, schema: Any) -> list[dict[str, str]]:
        return [
            {"label": "테스트", "sql": "SELECT bad_column FROM v_sales_order LIMIT 5"}
        ]
    ...
    async def retried_sql(
        ...
        calls.append(error)
        return [
            {"label": "테스트", "sql": "SELECT customer_id FROM v_sales_order LIMIT 5"}
        ]
...
def test_query_sales_raises_when_retry_also_fails() -> None:
    ...
    async def retried_sql(
        ...
        return [{"label": "테스트", "sql": "SELECT still_bad FROM v_sales_order LIMIT 5"}]
...
def test_query_sales_handles_compound_question_with_multiple_labels() -> None:
    ...
    with (
        ...
        evidence = asyncio.run(query_module.query_sales("올해 매출과 매출 최고 기업 알려줘"))

    assert len(evidence) == 2
    by_label = {item["label"]: item for item in evidence}
    assert by_label["올해 매출 합계"]["rows"] == [{"total": 1000}]
    assert by_label["매출 최고 기업"]["rows"] == [{"customer_name": "Acme"}]
    for item in evidence:
        assert item["domain"] == "sales"


def test_query_sales_keeps_per_item_empty_result_without_dropping_others() -> None:
    """한 항목만 빈 결과여도 다른 항목의 결과는 그대로 남는다(전부 비었을 때만 NO_RESULT는 server.py 책임)."""

    async def multi_sql(question: str, schema: Any) -> list[dict[str, str]]:
        return [
            {"label": "결과 있음", "sql": "SELECT 1 AS total FROM v_sales_order"},
            {
                "label": "결과 없음",
                "sql": "SELECT 1 AS total FROM v_sales_order WHERE 1=0",
            },
        ]
    ...
    with (
        ...
        evidence = asyncio.run(query_module.query_sales("복합질문"))
