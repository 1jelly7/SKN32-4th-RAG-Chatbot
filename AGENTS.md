...
- `mcp_servers/document_tools/`, `ingestion/`는 RAG 소유이고, `mcp_servers/data_tools/{purchase,sales}/`, `etl/{purchase,sales}/`, `database/{purchase,sales}/`는 도메인 소유다. 폐기된 도메인 별칭이나 중복 MCP 패키지를 다시 만들지 않는다. 담당 범위 밖에서는 소유자와 계약·생성 절차를 먼저 합의한다.
