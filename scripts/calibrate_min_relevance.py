...
TEST_CASES = [
    ...
    {
        "question": "법인카드 사용 금지 업소에 대해서 알려줘",
        "expected_keyword": "클린카드",
    },
...
async def main() -> None:
    ...
    print("=" * 70)
    print(
        "1) 관련 있는 질문들의 최고 점수 (이게 낮으면 min_relevance를 그 아래로 잡아야 함)"
    )
    print("=" * 70)
    positive_scores = []
    for case in TEST_CASES:
        ...
        matching = [
            r for r in results if case["expected_keyword"] in r.get("content", "")
        ]
        if matching:
            best = max(matching, key=lambda r: r["score"])
            positive_scores.append(best["score"])
            print(
                f"[{best['score']:.3f}] {case['question']!r} -> 정답 청크 찾음 (top_k 안에 있음)"
            )
        else:
            top_score = results[0]["score"]
            print(
                f"[찾지 못함, 1등 점수={top_score:.3f}] {case['question']!r} "
    ...
    print("=" * 70)
    print(
        "2) 무관한 질문들의 최고 점수 (이게 높으면 min_relevance를 그 위로 잡아야 함)"
    )
    print("=" * 70)
    negative_scores = []
    ...
    if positive_scores and negative_scores:
        ...
        if min_positive > max_negative:
            ...
            print(
                "(관련 있음/없음 점수 구간이 겹치지 않아 안정적으로 기준을 잡을 수 있습니다)"
            )
        else:
            print(
            ...
            print(
                "   즉 어떤 고정값을 넣어도 일부는 놓치거나 일부는 잘못 통과시킵니다."
            )
            print(
                f"   일단 재현율(누락 방지) 우선이면 {max(0.0, min_positive - 0.05):.2f} 정도로,"
            )
            print(
                f"   정확도(오답 방지) 우선이면 {max_negative + 0.05:.2f} 정도로 잡는 걸 권장합니다."
            )
    else:
        print(
            "점수를 비교할 데이터가 부족합니다. TEST_CASES/NEGATIVE_CASES를 확인하세요."
        )
