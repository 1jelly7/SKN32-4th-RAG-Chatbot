...
from etl.sales.config import MySQLWriteConfig
from etl.sales.ddl import build_create_table_sql
...
class SalesETLMySQLClient:
    ...
    - MYSQL_WRITE_* 계정(etl_writer)만 사용한다. 조회용 read-only 계정과는
      완전히 분리되어 있다 (mcp_servers/data_tools/sales/mysql.py 소유).
    - SALES_SCHEMA(etl/sales/schema.py)에 정의되지 않은 테이블은 거부한다.
    - PK 충돌 시 INSERT ... ON DUPLICATE KEY UPDATE로 처리해 동일 원천을
      재실행해도 행 수가 늘어나지 않는다 (ETL 멱등성).
    - `sales` 데이터베이스 자체는 이 클라이언트가 만들지 않는다. 프로젝트를
      처음 설정하는 사람이 `database/sales/create_sales_db.sql`을 1회 실행해
      만들어둔다는 것을 전제로 한다.
    """
    ...
    def ensure_table(self, table: str) -> None:
        """대상 테이블이 없으면 database/sales/ddl.sql 기준으로 생성한다."""
        if table not in SALES_SCHEMA:
            raise ValueError(f"'{table}'은(는) 판매 도메인 허용 테이블이 아닙니다.")
        conn = self._connect()
        try:
    ...
    def upsert(self, frame: pd.DataFrame, table: str) -> LoadResult:
        ...
        columns = [c["name"] for c in SALES_SCHEMA[table]["columns"]]
        pk = SALES_SCHEMA[table]["pk"]
