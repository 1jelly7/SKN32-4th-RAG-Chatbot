...
def build_steps(arguments: argparse.Namespace) -> list[SetupStep]:
    ...
    if not arguments.skip_django:
        steps.append(
            SetupStep("Django migrations", [python, "django_app/manage.py", "migrate"])
        )
    ...
    if not arguments.skip_documents:
        steps.extend(
            [
                ...
                SetupStep(
                    "FAISS document indexing", [python, "scripts/ingest_documents.py"]
                ),
            ]
        )
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Django·문서·구매·판매 초기화 orchestration"
    )
    parser.add_argument(
        "--apply", action="store_true", help="계획만 출력하지 않고 실제 명령을 실행"
    )
    parser.add_argument(
        "--skip-django", action="store_true", help="Django migration 단계를 생략"
    )
    parser.add_argument(
        "--skip-documents",
        action="store_true",
        help="문서 등록·FAISS 인덱싱 단계를 생략",
    )
    parser.add_argument(
        "--skip-purchase", action="store_true", help="구매 ETL 단계를 생략"
    )
    parser.add_argument(
        "--skip-sales", action="store_true", help="판매 ETL 단계를 생략"
    )
    ...
    parser.add_argument(
        "--create-superuser",
        action="store_true",
        help="migration 뒤 Django 대화형 관리자 생성",
    )
...
def validate_apply_preconditions(arguments: argparse.Namespace) -> None:
    ...
    if not (PROJECT_ROOT / ".env").is_file():
        raise FileNotFoundError(
            ".env 파일이 없습니다. .env.example을 복사하고 연결 정보를 설정하세요."
        )
